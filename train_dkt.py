...
from dkt_model import DKTModel, build_dataset_from_csv, collate_fn, train_epoch, evaluate, save_model

# Configurar logging
logging.basicConfig(
    level=logging.INFO,
    format='{"timestamp":"%(asctime)s","level":"%(levelname)s","module":"%(name)s","message":"%(message)s"}',
    handlers=[logging.FileHandler("results/logs/train.log"), logging.StreamHandler(sys.stdout)],
)
...
def main():
    ...
    parser.add_argument(
        "--data-csv", type=str, default="data/real_combined_dataset.csv", help="Caminho do CSV de dados"
    )
    ...
    parser.add_argument("--hidden-size", type=int, default=128, help="Tamanho da camada oculta")
    parser.add_argument("--n-layers", type=int, default=2, help="Número de camadas LSTM")
    parser.add_argument("--dropout", type=float, default=0.2, help="Taxa de dropout")
    parser.add_argument("--max-seq-len", type=int, default=200, help="Comprimento máximo da sequência")
    parser.add_argument("--device", type=str, default="cpu", help="Dispositivo (cpu ou cuda)")
    parser.add_argument("--seed", type=int, default=42, help="Seed para reproducibilidade")
    parser.add_argument("--save-dir", type=str, default="models", help="Diretório para salvar modelo")
    parser.add_argument("--resume", type=str, default=None, help="Caminho de checkpoint para retomar treino")
    parser.add_argument("--val-split", type=float, default=0.2, help="Proporção de dados para validação")
    ...
    # Carregar dados
    dataset, item_to_idx, metadata = build_dataset_from_csv(args.data_csv, max_seq_len=args.max_seq_len)
    ...
    model = DKTModel(n_items=n_items, hidden_size=args.hidden_size, n_layers=args.n_layers, dropout=args.dropout)
