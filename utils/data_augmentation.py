...
class FeatureEngineer:
    ...
    def add_temporal_features(df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['correct_streak'] = df.groupby('student_id')['correct'].apply(
    ...
    def add_aggregated_features(df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['student_accuracy_so_far'] = df.groupby('student_id')['correct'].apply(
    ...
    def create_skill_features(df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['skill_performance'] = df.groupby(['student_id', 'skill_id'])['correct'].apply(
            lambda x: x.expanding().mean()
        ).reset_index(drop=True)
        
        # Número de skills únicas vistas
        df['n_skills_seen'] = df.groupby('student_id')['skill_id'].apply(
            lambda x: x.expanding().apply(lambda y: len(set(y)), raw=False)
        ).reset_index(drop=True)
