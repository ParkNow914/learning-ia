...
class PredictionCache:
    ...
    def __init__(self, cache_dir: str = "cache", max_cache_size_mb: int = 100, ttl_seconds: int = 3600):
        """
        Inicializa cache.
    ...
    def _generate_key(self, student_history: list, candidate_item: str) -> str:
        ...
        history_str = json.dumps(
            {"history": [(h["item_id"], h["correct"]) for h in student_history], "candidate": candidate_item},
            sort_keys=True,
        )

        return hashlib.md5(history_str.encode()).hexdigest()

    def get(self, student_history: list, candidate_item: str) -> Optional[float]:
        """
    ...
    def put(self, student_history: list, candidate_item: str, probability: float):
        """
        ...
        # Atualizar metadata
        self.metadata["entries"][key] = {"timestamp": time.time(), "size": file_size}
    ...
    def _evict_old_entries(self):
        ...
        sorted_entries = sorted(self.metadata["entries"].items(), key=lambda x: x[1]["timestamp"])
...
class BatchInferenceOptimizer:
    ...
    @staticmethod
    def prepare_batch(
        student_histories: list, candidate_items: list, item_to_idx: Dict, max_seq_len: int = 200
    ) -> Tuple[Any, Any]:
        """
    ...
    @staticmethod
    def batch_predict(model: Any, inputs: Any, batch_size: int = 32, device: str = "cpu") -> Any:
        """
